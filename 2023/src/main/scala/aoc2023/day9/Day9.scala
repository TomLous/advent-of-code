...
object Day9 extends App:
  ...
  private def parse(lines: Iterator[String]) = lines.map{
    case s"$line" => line.split(" ").map(_.toLong).toList
  }.toList
   ...
   private def predictNext(history: List[Long]):Long =
      val diffs = history.sliding(2).map{
        case List(a,b) => b-a
      }.toList

      if diffs.forall(_ == 0) then
        history.last
      else
        history.last + predictNext(diffs)
   ...
   private def predictPredecessor(history: List[Long]):Long =
     val diffs = history.sliding(2).map{
      ...
      if diffs.forall(_ == 0) then
        ...
        history.head - predictPredecessor(diffs)
