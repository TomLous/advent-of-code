...
object model {
  ...
  case class Zone(sensorBeacons: List[(Point, Point)]):
    ...
    lazy val (minCol, maxCol) = sensorRange
      .foldLeft((0L, 0L)) { case ((minCol, maxCol), (sensor, range)) =>
        val colPos = List(sensor.col - range, sensor.col + range, minCol, maxCol)
        (colPos.min, colPos.max)
      }

    def countCovered(row: Long): Long =
