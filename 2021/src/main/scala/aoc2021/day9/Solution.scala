...
object Solution {
  ...
  val findLowestPoint = ZPipeline.mapChunks[Chunk[List[Int]], (Int,Long)](_.flatMap(_.toList match
    case previousLine :: currentLine :: nextLine :: Nil =>
      Chunk.fromIterator(
        (None +: currentLine.zipWithIndex.map(Some(_)) :+ None).sliding(3).flatMap{
          case previousItem :: Some(value, pos) :: nextItem :: Nil =>
            ...
            if(List(up, down, left, right).flatten.forall(_ > value)) Some(value, pos.toLong) else None
        }
      )
  ...
  def parseInput(lineStream: ZStream[Any, Throwable, String]): ZIO[Any, Throwable, List[Input]] =
    (ZStream.fromChunk(Chunk(List.empty[Int])) ++ lineStream
      ...
      .zipWithIndex
      .map{
        case ((value, x), y) => LowPoint(value, x, y)
      }
      .runCollect
      .map(_.toList)
